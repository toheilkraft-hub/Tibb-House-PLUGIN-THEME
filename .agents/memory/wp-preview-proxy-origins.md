...
**How to apply:** Preserve `X-Forwarded-Proto` and `X-Forwarded-Host` for real Replit requests. For local proxy requests, rewrite HTML-only internal absolute origins to relative URLs without altering public responses. If Vite uses `selfHandleResponse`, copy upstream response headers before piping or rewriting; otherwise CSS can arrive without a usable MIME type and the panel shows raw HTML.